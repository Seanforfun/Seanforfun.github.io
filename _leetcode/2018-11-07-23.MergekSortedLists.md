...
* Method 1: divide and conquer
   ...
   */
  class Solution {
      // we use divide and conquer to solve this question
      public ListNode mergeKLists(ListNode[] lists) {
      ...
      private ListNode merge(ListNode[] lists, int low, int high){
          ...
          ListNode lowList = merge(lists, low, mid);
          ListNode highList = merge(lists, mid + 1, high);
          return merge2Lists(lowList, highList);
      }
      private ListNode merge2Lists(ListNode first, ListNode second){
          if(first == null && second == null) return null;
          else if(first != null && second != null){
              ListNode node = first.val <= second.val ? first: second;
              if(first.val <= second.val) node.next = merge2Lists(first.next, second);
              else node.next = merge2Lists(first, second.next);
              return node;
          }else{
